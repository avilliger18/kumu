...
export const nutrientPanel = v.object({
  ...
  additionalNutrients: v.optional(v.array(v.object({
    name:  v.string(),
    value: v.number(),
    unit:  v.string(),
  }))),
...
export const additive = v.object({
  ...
  riskLevel: v.union(v.literal("low"), v.literal("moderate"), v.literal("high"), v.literal("unknown")),
  notes:     v.optional(v.string()),
});

export const qualityScores = v.object({
  nutriScore:        v.optional(v.union(v.literal("A"), v.literal("B"), v.literal("C"), v.literal("D"), v.literal("E"))),
  novaGroup:         v.optional(v.union(v.literal(1), v.literal(2), v.literal(3), v.literal(4))),
...
export default defineSchema({
  ...
  productBatches: defineTable({
    ...
    recallStatus:  v.union(v.literal("none"), v.literal("active"), v.literal("resolved")),
  ...
  trackingEvents: defineTable({
    ...
    occurredAt:     v.number(),
    actorType:      v.union(v.literal("system"), v.literal("producer"), v.literal("admin")),
